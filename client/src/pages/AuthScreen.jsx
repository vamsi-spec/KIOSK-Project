...
export default function AuthScreen() {
  ...
  const handleKey = (key) => {
    if (key === '⌫') {
      ...
      // empty cell in numpad grid — do nothing
  ...
  return (
    <KioskLayout showBack={true} showHome={false}>
      <div className="flex flex-col items-center justify-center h-full px-8 gap-8">
        ...
        <div className={`w-full max-w-sm rounded-2xl border-2 px-6 py-5
          ...
          {errMsg && (
            <p className="text-red-500 text-base mt-2">{errMsg}</p>
          )}
        </div>
